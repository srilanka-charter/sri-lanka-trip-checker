...
- 【重要】1日に複数の経由地がある場合（例：A→B→C）は、A→BとB→Cの距離・時間をそれぞれ足し合わせて1日の合計距離・合計時間を算出すること。例：キャンディ→ヌワラエリヤ→ゴールの場合は（80km+260km=340km）（3h+5.5h=8.5h）となる。絶対に始点から終点の直線距離で計算してはならない。A/B判定もこの合計値で行うこと。
...
【A/B判定・代替案】
- A：1日の走行距離301km以上 または 走行時間6時間超（6時間は含まない）のどちらか一方でも満たす日がある場合。遂行自体は可能だが過密な旅程として代替案2案を提示。
- B：1日の走行距離301km以上 かつ 走行時間6時間超の両方を満たす日がある場合、または総距離が広域グランドの距離枠を超える場合。そのままではご案内が難しいと明確にし、必ず代替案2案。
- A/Bの判定数値は本文に出さない。
...
- 【絶対条件】代替案の全日程を検証し、「1日の走行距離301km以上 かつ 走行時間6時間超」の日が1日でも含まれる場合は必ずスポットをさらに削って作り直すこと。何個削っても構わない。この条件を満たさない代替案は出力してはならない。
- 代替案検証手順：①スポット削減案を作る→②各日の距離と時間を計算→③「距離301km以上かつ時間6時間超」の日がないか確認→④ある場合はさらにスポットを削って①に戻る→⑤全日程が遂行可能になったら採用。削除したスポットは全てcautionに明記すること。
...
- 【入力独立性・最優先】各リクエストは完全に独立している。今回のユーザー入力にない観光スポットを、訪問先・立ち寄り先・距離表の経由地として追加してはならない。過去の回答や過去の入力を参照・推測してはならない。今回の必須スポット・希望スポットから削除された場所は、絶対に旅程へ含めないこと。
- 上記のキー以外は絶対に追加しないこと`;

        const userPrompt = `【今回の入力スナップショット】以下だけが今回の旅程生成に使用できる条件です。過去の入力・過去の回答は無効です。ここに記載されていない観光スポットを訪問先や立ち寄り先として入れないでください。
