...
【A/B判定・代替案】
- A：1日の走行距離300km超 かつ 走行時間6時間超の両方を満たす場合のみ。どちらか一方のみ（距離超過のみ、または時間超過のみ）は遂行可能とする。
...
- 【絶対条件】代替案の全日程を検証し、「1日の走行距離300km超 かつ 走行時間6時間超」の日が1日でも含まれる場合は必ずスポットをさらに削って作り直すこと。何個削っても構わない。この条件を満たさない代替案は出力してはならない。
- 代替案検証手順：①スポット削減案を作る→②各日の距離と時間を計算→③「距離300km超かつ時間6時間超」の日がないか確認→④ある場合はさらにスポットを削って①に戻る→⑤全日程が遂行可能になったら採用。削除したスポットは全てcautionに明記すること。
